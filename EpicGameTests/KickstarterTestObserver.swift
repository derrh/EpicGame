// See: https://github.com/kickstarter/Kickstarter-ReactiveExtensions
...
internal final class TestObserver <Value, Error: Swift.Error> {
  typealias Event = Signal<Value, Error>.Event
  typealias Observer = Signal<Value, Error>.Observer
  
  internal private(set) var events: [Event] = []
  internal private(set) var observer: Observer!
  
  internal init() {
    self.observer = Observer(action)
  }
  ...
  internal var failedError: Error? {
    return self.events.flatMap { $0.error }.first
  }
  ...
  internal var didInterrupt: Bool {
    return self.events.filter {
      guard  case .interrupted = $0 else { return false }
      return true
      }.count > 0
  }
...
extension Signal {
  var testObserver: TestObserver<Value, Error> {
    let obs = TestObserver<Value, Error>()
...
extension SignalProducer {
  var testObserver: TestObserver<Value, Error> {
    let test = TestObserver<Value, Error>()
    start(test.observer)
    return test
  }
}
